...
% -----------------------------------------------------------------------------
% preferInput 宏（方案A：以 polished 为主线）
% 约定的版本优先级（从高到低）：
%   1) *_polished_unified.tex     —— 逐章风格统一后的最终稿（当前主线）
%   2) *_polished.tex             —— 对照式定稿（你项目现用主线）
%
% 依赖：仅使用 LaTeX 内核 \IfFileExists（无需 ifthen 等宏包）
% 日志：每次选择会在编译日志中输出 [preferInput] -> Using ... 便于排查
% 用法：\preferInput{chapters/00-Intro}
% -----------------------------------------------------------------------------
\makeatletter
\newcommand{\preferInput}[1]{%
  \begingroup
  % 基础名（不含后缀），例如 chapters/00-Intro
  \def\@base{#1}%
  % 预构造完整文件名（含 .tex 扩展名），避免展开期拼接问题
  \edef\@A{\@base_polished_unified.tex}%
  \edef\@B{\@base_polished.tex}%
  \edef\@E{\@base.tex}%
  %
  % 按优先级依次查找并输入
  \IfFileExists{\@A}{%
    \typeout{[preferInput] -> Using \@A}%
    \input{\@A}%
  }{%
    \IfFileExists{\@B}{%
      \typeout{[preferInput] -> Using \@B}%
      \input{\@B}%
    }{%
      \IfFileExists{\@C}{%
        \typeout{[preferInput] -> Using \@C}%
        \input{\@C}%
      }{%
        \IfFileExists{\@D}{%
          \typeout{[preferInput] -> Using \@D}%
          \input{\@D}%
        }{%
          \typeout{[preferInput] -> Fallback to \@E}%
          \input{\@E}%
        }%
      }%
    }%
...
\preferInput{chapters/2-History-Math.v3.2-polished} % 智能，生于数学
